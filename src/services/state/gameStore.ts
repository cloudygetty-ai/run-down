...
function scatterLoot(count: number): LootDrop[] {
  ...
  return Array.from({ length: count }, (_, i) => ({
    id: `loot_${i}`,
    position: { x: randomInRange(50, MAP_WIDTH - 50), y: randomInRange(50, MAP_HEIGHT - 50) },
    weapon: makeWeapon(types[randomInt(0, types.length - 1)], rarities[randomInt(0, rarities.length - 1)]),
...
function buildInitialState(): GameState {
  const human = makePlayer('human', 'You', true, { x: MAP_WIDTH / 2, y: MAP_HEIGHT / 2 });
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  placeBuildPiece: (piece: BuildPiece) => {
    ...
    const players = gameState.players.map((p) => {
      ...
      return { ...p, materials: { ...p.materials, [mat]: p.materials[mat] - matCost } };
    });
