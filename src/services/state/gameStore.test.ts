...
describe('gameStore.pickUpLoot', () => {
  ...
  it('adds loot weapon into first empty weapon slot', () => {
    ...
    const updatedHuman = useGameStore.getState().gameState.players.find((p) => p.isHuman)!;
    // Slot 1 or 2 should now have a weapon (slot 0 = pickaxe)
    const hasWeapon = updatedHuman.weapons[1] !== null || updatedHuman.weapons[2] !== null;
  ...
  it('adds shield from loot to player shield', () => {
    ...
    const updated = useGameStore.getState().gameState.players.find((p) => p.isHuman)!;
  ...
  it('does nothing for a loot id that does not exist', () => {
    ...
    expect(useGameStore.getState().gameState.lootDrops).toHaveLength(gameState.lootDrops.length);
  });
});
...
describe('gameStore.placeBuildPiece', () => {
  ...
  it('does not place if player has insufficient materials', () => {
    const { gameState, updateGameState, placeBuildPiece } = useGameStore.getState();
    const human = gameState.players.find((p) => p.isHuman)!;
    // Zero out wood
