import { Player, Weapon, GameState } from '../../types';
import { logger } from '../../utils';

const RELOAD_INTERVAL_MS = 100;
...
export function startReload(
  ...
  if (!weapon || weapon.type === 'pickaxe' || weapon.isReloading) return;
...
export function switchWeaponSlot(player: Player, slot: 0 | 1 | 2): Player {
  if (slot === player.activeWeaponSlot) return player;
...
export function canFire(weapon: Weapon, lastFireTimeMs: number, nowMs: number): boolean {
  if (weapon.currentAmmo <= 0 || weapon.isReloading) return false;
...
export function computeAimPoint(from: { x: number; y: number }, to: { x: number; y: number }, weapon: Weapon): { x: number; y: number } {
