...
export function resolvePlayerWallCollision(player: Player, pieces: BuildPiece[]): Vector2 {
...
export function isPlayerHitByBullet(
  ...
  const t = Math.max(
    ...
    Math.min(
      1,
      ((target.position.x - bulletOrigin.x) * dx + (target.position.y - bulletOrigin.y) * dy) /
        lenSq,
    ),
  );
  const closest = {
