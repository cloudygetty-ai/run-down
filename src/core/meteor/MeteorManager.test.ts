...
describe("MeteorManager", () => {
  ...
  describe("tickBombardment", () => {
    ...
    it("begins shrinking when countdown expires", () => {
      const b = createInitialBombardment(MAP_W, MAP_H);
      const { bombardment } = tickBombardment(
        b,
        b.timeUntilNextPhase + 100,
        MAP_W,
        MAP_H
      );
    ...
    it("spawns impact OUTSIDE the shelter zone", () => {
      const b = createInitialBombardment(MAP_W, MAP_H);
      const earlyImpact = { ...b, timeUntilNextImpact: 10 };
      const { newImpacts, bombardment } = tickBombardment(
        earlyImpact,
        50,
        MAP_W,
        MAP_H
    ...
    it("ages existing impacts on each tick", () => {
      const b = createInitialBombardment(MAP_W, MAP_H);
      const earlyImpact = { ...b, timeUntilNextImpact: 10 };
      const { bombardment: after1 } = tickBombardment(
        earlyImpact,
        50,
        MAP_W,
        MAP_H
      );
      const { bombardment: after2 } = tickBombardment(
        after1,
        500,
        MAP_W,
        MAP_H
    ...
    it("removes impacts that have exceeded maxAge", () => {
      ...
      expect(
        bombardment.activeImpacts.find((i) => i.id === "old")
      ).toBeUndefined();
    });
  });
});
