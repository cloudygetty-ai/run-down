...
const THRESHOLDS: Threshold[] = [
  ...
  { label: 'DEVICE DETECTED', max: Infinity, color: '#EF4444', desc: 'Camera or surveillance device' },
];
...
export function MagnetometerScreen() {
  ...
  useEffect(() => {
    if (!hot) { pulseAnim.setValue(1); return; }
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        >
          <Text style={[styles.anomalyNum, { color: threshold.color }]}>
            {anomaly.toFixed(1)}
          </Text>
          <Text style={styles.anomalyUnit}>µT ANOMALY</Text>
          <Text style={[styles.thresholdLabel, { color: threshold.color }]}>
            {threshold.label}
          </Text>
        ...
        <View style={styles.xyzRow}>
          {(['x', 'y', 'z'] as const).map(axis => (
            <View key={axis} style={styles.xyzCard}>
              ...
              <Text style={styles.xyzVal}>
                {(magneticReading?.[axis] ?? 0).toFixed(1)}
              </Text>
        ...
        <View style={styles.statsRow}>
          <StatCard label="MAGNITUDE"  value={`${(magneticReading?.magnitude ?? 0).toFixed(1)} µT`} />
...
const sc = StyleSheet.create({
  ...
  val: { color: '#C9A84C', fontSize: 13, fontFamily: 'monospace', fontWeight: '700', marginBottom: 4, textAlign: 'center' },
  label: { color: '#3D3650', fontSize: 8, letterSpacing: 2, fontFamily: 'monospace', textAlign: 'center' },
});
...
const styles = StyleSheet.create({
  ...
  anomalyUnit: { color: '#3D3650', fontSize: 10, letterSpacing: 4, fontFamily: 'monospace', marginTop: 4 },
  thresholdLabel: { fontSize: 12, letterSpacing: 4, fontFamily: 'monospace', fontWeight: '700', marginTop: 14 },
  ...
  xyzAxis: { color: '#5C566E', fontSize: 9, letterSpacing: 3, fontFamily: 'monospace', marginBottom: 4 },
  ...
  guide: { backgroundColor: '#0A0814', borderRadius: 12, padding: 16, marginTop: 16, marginBottom: 16 },
  guideTitle: { color: '#3D3650', fontSize: 9, letterSpacing: 4, fontFamily: 'monospace', marginBottom: 14 },
  guideRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
  ...
  tipsTitle: { color: '#C9A84C', fontSize: 9, letterSpacing: 4, fontFamily: 'monospace', marginBottom: 12 },
